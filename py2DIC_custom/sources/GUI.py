...
class First(QDialog):
    def __init__(self, parent=None):
        ...
        pixmap = QPixmap(path+'../undeformed.png')
        ...
        self.le_pixel_dimension.setText("0.03")
    ...
    def button_click(self):
        ...
        try:
            ...
            changes.append("Collected user parameters in GUI.")
        ...
        try:
            ...
            # New logic: get reference and deformed images
            reference_image = os.path.join(prova, 'undeformed.jpg')
            deformed_images = sorted(glob.glob(os.path.join(prova, 'deformed_*.jpg')))
            changes.append(f"Reference image: {reference_image}")
            changes.append(f"Deformed images: {deformed_images}")

            log_message = DIC_main.DIC(
                reference_image, deformed_images, self.dim_pixel, start_index, levels, image_time_sampling,
                templateWidth, b, d, rectangley1, rectangley2, rectanglex1, rectanglex2, H, V, defor, ml
            )
            changes.append("Called DIC_main.DIC with reference and deformed images list.")
            ...
            with open("changelog_py2DIC.txt", "a") as f:
                f.write("\n--- GUI.py changes (button_click) ---\n")
                for c in changes:
                    f.write(c + "\n")
            return True
        except Exception as e:
            print("Error during cross correlation computation", e)
            self.display.setText(self.display.toPlainText() + "Error during cross correlation computation\n")
            with open("changelog_py2DIC.txt", "a") as f:
                f.write("\n--- GUI.py error (button_click) ---\n")
                f.write(str(e) + "\n")
            return False
...
#For finer displacement and strain results:
#Set Template width [pixel] to 7 or 9.
#Set Edge y [pixel] and Edge x [pixel] to lower values (e.g., y=7, x=1).
#Set Multilook [pixel] to a lower value (e.g., 5 or 3).