...
def DIC(reference_image, deformed_images, dim_pixel, start_index, levels, image_time_sampling , temp_dim, b, d, recty1, recty2, rectx1, rectx2, H, V, defor, ml, c=0):
    """
    Modified to accept a reference image and a list of deformed images.
    """
    changelog = []
    ...
    test_name = os.path.basename(os.path.dirname(reference_image))
    ...
    # Use the provided reference and deformed images
    img_names = [reference_image] + deformed_images
    changelog.append(f"DIC now uses reference: {reference_image} and deformed: {deformed_images}")

    # Read the reference image to obtain information about the camera resolution
    img1 = cv2.imread(reference_image, 0)
    crop_img1 = img1[:, :]
    ...
    h = int(Dim_y / (temp_dim + 2 * b + c) - 1)
    w = int(Dim_x / (temp_dim + 2 * d + c) - 1)
    grid_points = H * V * w * h

    # Collect all results for Excel export
    all_results = {}
    summary_rows = []
    for idx, deformed_image in enumerate(deformed_images):
        msg = ''
        msg = msg + '-----------'
        msg = msg + "\nAnalysed images"
        msg = msg + "\n1: " + os.path.basename(str(reference_image))
        msg = msg + "\n2: " + os.path.basename(str(deformed_image))
        print(msg)

        img1 = cv2.imread(reference_image, 0)
        crop_img1 = img1[recty1:recty2, rectx1:rectx2]
        img2 = cv2.imread(deformed_image, 0)
        ...
        if os.path.exists(os.path.split(reference_image)[0] + "\\Mask"):
            mask_read = cv2.imread(os.path.split(reference_image)[0] + "\\Mask\\mask.png", 0)
        ...
        img_key = os.path.splitext(os.path.basename(deformed_image))[0]
        all_results[f'{img_key}_Ux_Raw'] = dx
        all_results[f'{img_key}_Uy_Raw'] = dy
        all_results[f'{img_key}_Ux_Smoothed'] = dispx_smoothed
        all_results[f'{img_key}_Uy_Smoothed'] = dispy_smoothed
        ...
        if defor:
            ...
            all_results[f'{img_key}_Strain_XX'] = strain_xx2
            all_results[f'{img_key}_Strain_YY'] = strain_yy2
            all_results[f'{img_key}_Strain_XY'] = strain_xy2

        # Add summary row for this image
        summary_rows.append({
            'Deformed Image': os.path.basename(deformed_image),
            'Pixel Dimension (mm)': dim_pixel,
            'Template Width': temp_dim,
            'Grid Points': grid_points,
            'Image Resolution': f"{Dim_x}x{Dim_y}"
        })

        # ...existing code for text and plots (unchanged)...
        out_prefix = f"OutputPlots/{test_name}_results_{img_key}"
        with open(out_prefix + "_dy.txt", "w") as file_stats:
            for i in range(np.shape(dy)[0]):
                for j in range(np.shape(dy)[1]):
                    file_stats.write(str(dy[i, j]) + '\t')
                file_stats.write('\n')
        with open(out_prefix + "_dx.txt", "w") as file_stats:
            for i in range(np.shape(dx)[0]):
                for j in range(np.shape(dx)[1]):
                    file_stats.write(str(dx[i, j]) + '\t')
                file_stats.write('\n')

        # ...existing code for plots (unchanged)...
        fig = plt.figure(f"Displacements smoothed dx for {os.path.basename(deformed_image)}")
        ...
        plt.imshow(dispx_smoothed, cmap=cm.jet, alpha=0.5)
        cb = plt.colorbar()
        if dim_pixel == 1:
            cb.set_label('px')
        else:
            cb.set_label('mm')
        dx_png_name = f"OutputPlots/{os.path.splitext(os.path.basename(deformed_image))[0]}_smoothed_dx.png"
        plt.savefig(dx_png_name, bbox_inches='tight')
        plt.close(fig)

        fig = plt.figure(f"Displacements smoothed dy for {os.path.basename(deformed_image)}")
        plt.title("Smoothed vertical displacements: v")
        plt.gca().set_aspect('equal', adjustable='box')
        plt.imshow(crop_img1[int((temp_dim - 1) / 2 + b)::, int((temp_dim - 1) / 2 + d)::], cmap=plt.cm.gray, origin='upper')
        plt.imshow(dispy_smoothed, cmap=cm.jet, alpha=0.5)
        cb = plt.colorbar()
        if dim_pixel == 1:
            cb.set_label('px')
        else:
            cb.set_label('mm')
        dy_png_name = f"OutputPlots/{os.path.splitext(os.path.basename(deformed_image))[0]}_smoothed_dy.png"
        plt.savefig(dy_png_name, bbox_inches='tight')
        plt.close(fig)

        # Strain heatmap (example for strain_xx2)
        if defor:
            # Strain XX
            fig = plt.figure(f"Strain XX {os.path.basename(deformed_image)}")
            plt.title("Strain XX (εxx)")
            plt.gca().set_aspect('equal', adjustable='box')
            plt.imshow(strain_xx2, cmap=cm.jet, origin='upper')
            cb = plt.colorbar()
            cb.set_label('strain')
            strain_xx_png_name = f"OutputPlots/{os.path.splitext(os.path.basename(deformed_image))[0]}_strain_xx.png"
            plt.savefig(strain_xx_png_name, bbox_inches='tight')
            plt.close(fig)

            # Strain YY
            fig = plt.figure(f"Strain YY {os.path.basename(deformed_image)}")
            plt.title("Strain YY (εyy)")
            plt.gca().set_aspect('equal', adjustable='box')
            plt.imshow(strain_yy2, cmap=cm.jet, origin='upper')
            cb = plt.colorbar()
            cb.set_label('strain')
            strain_yy_png_name = f"OutputPlots/{os.path.splitext(os.path.basename(deformed_image))[0]}_strain_yy.png"
            plt.savefig(strain_yy_png_name, bbox_inches='tight')
            plt.close(fig)

            # Strain XY
            fig = plt.figure(f"Strain XY {os.path.basename(deformed_image)}")
            plt.title("Strain XY (εxy)")
            plt.gca().set_aspect('equal', adjustable='box')
            plt.imshow(strain_xy2, cmap=cm.jet, origin='upper')
            cb = plt.colorbar()
            cb.set_label('strain')
            strain_xy_png_name = f"OutputPlots/{os.path.splitext(os.path.basename(deformed_image))[0]}_strain_xy.png"
            plt.savefig(strain_xy_png_name, bbox_inches='tight')
            plt.close(fig)


    # plt.show()  # Disabled to prevent displaying plots
    stop = timeit.default_timer()
    print('Time [min]', (stop - start) / 60)

    try:
        excel_filename = "OutputPlots/DIC_Results.xlsx"
        stats_rows = []
        for key, data in all_results.items():
            arr = np.array(data)
            # Only process 2D arrays (displacement/strain fields)
            if arr.ndim == 2:
                stats_rows.append({
                    'Field': key,
                    'Max': np.nanmax(arr),
    ...
    # Write changelog
    with open("changelog_py2DIC.txt", "a") as f:
        f.write("\n--- Main.py changes (DIC) ---\n")
        for c in changelog:
            f.write(c + "\n")
    return msg
